def Taiwan_ID_Number_Check(ID):
    ...
    while True:
        if len(ID) != 10:               #字數檢查 length check
            return False
            break
        elif not ID[1:9].isnumeric():   #2～10碼檢查 [1:9] not str check 
            return False
            break
    
        elif ID[0].isnumeric():         #1碼檢查 [0] not int check
            return False
            break
    
        elif len(ID) == 10:             #驗證 (N0 十位數 + (N0 個位數 x 9) + (N1 x 8) + (N2 x 7) +  (N3 x 6) +  (N4 x 5) +  (N5 x 4) +  (N6 x 3) +  (N7 x 2) + N8 + N9)
            ID = ID.lower()
            ID = list(ID)
            ID[0] = first[ID[0]]
            ID00 = ID[0]
